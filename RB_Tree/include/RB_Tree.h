...
private:
  struct Node {
    ...
    Node(const KeyT &_value) : value(_value) {}
    ...
    Node *get_bro() { return is_left_child() ? parent->right : parent->left; }
  };
...
private:
  void dump_regular_nodes(Node *node, std::ostream &dump) {
    ...
    dump << "\t" << reinterpret_cast<size_t>(node)
         << "[shape = Mrecord, fillcolor = \"" << (node->is_red ? red_ : black_)
  ...
  void compile_dot(const std::string &file_name) {
    std::string cmd =
        "dot -Tpng " + file_name + ".dot -o " + file_name + ".png -Gdpi=100\n";
...
public:
  ...
  RB_Tree(RB_Tree &&other) noexcept
      : root_(std::move(other.root_)), data_(std::move(other.data_)) {
